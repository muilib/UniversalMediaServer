...
//	private boolean subsAreValidForStreaming = false;
	...
	public Player resolvePlayer(RendererConfiguration renderer) {
		...
		for (DLNAMediaSubtitle subtitlesTrack : media.getSubtitleTracksList()) {
			hasEmbeddedSubs |= subtitlesTrack.isEmbedded();
			if (hasEmbeddedSubs) {
				break;
			}
		}
					...
					if (player != null) {
						...
						if (
							...
							if (mediaRenderer.isAccurateDLNAOrgPN()) {
								...
								params.mediaRenderer = mediaRenderer; //TODO: (Nad) Verify right calls/logic
								Player.setAudioAndSubs(getFileName(), media, params);
